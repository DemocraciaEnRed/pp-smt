...
class HomePropuestas extends Component {
  ...
  getQueryParams() {
    const { archive, years, params: { forum } } = this.props;
    const tipo = forum === 'catalogo' ? ['ganador'] : defaultValues.tipoIdea
    const queryString = window.location.search;
    const urlParams = new URLSearchParams(queryString);
    const selectedDistricts = urlParams.getAll('distrito')
    const selectedTags = urlParams.getAll('tag');
    const selectedTipoIdea = urlParams.getAll('tipoIdea') || tipo
    const selectedYears = urlParams.getAll('years');
    const selectedKwords = urlParams.get('kwords') || '';

    this.setState({
      zona: selectedDistricts.length ? selectedDistricts : defaultValues.zona,
      tag: selectedTags.length ? selectedTags : defaultValues.tag,
      tipoIdea: selectedTipoIdea.length ? selectedTipoIdea : tipo,
      years: selectedYears.length ? selectedYears : years || [],
      kwords: selectedKwords || undefined
    })
  }

  setQueryParams() {
    const { zona, tag, tipoIdea, years, kwords } = this.state
    const query = {
      distrito: zona.length ? zona : undefined,
      tag: tag.length ? tag : undefined,
      tipoIdea: tipoIdea.length ? tipoIdea : undefined,
      years: years.length ? years : undefined,
      kwords: kwords || undefined
    }
    browserHistory.push({
    ...
    return (
      <div>
        {forum && <h4 className="topics-title">{
          !forum.config.stage === 'seguimiento' ?
