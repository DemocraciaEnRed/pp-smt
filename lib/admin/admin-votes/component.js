...
export default class AdminVotes extends Component {
  ...
  fetchVotes() {
    ...
    voteStore.findByTopic(this.props.topic.id)
      .then(votes => {
        ...
        this.setState({ votes, votesByBox, loadingVotes: false })
      })
  }
  ...
  render() {
    const { zones, votes, votesByBox, ballotBox, quantity, currentPage, votesPerPage, modalOpen, loadingVotes } = this.state
    ...
    return (
      <div className='admin-votes'>
        <h2>Votos</h2>
        {loadingVotes && <div><i className="icon-spinner icon-spin"></i><spam> Cargando votos...</spam></div>}
